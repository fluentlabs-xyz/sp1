...
use sp1_rwasm_executor::{syscalls::SyscallCode, ExecutionReport, Opcode};

use crate::riscv::RwasmAirDiscriminants;
...
pub trait CostEstimator {
    ...
    fn estimate_gas(&self) -> u64 {
        ...
        let cpu_gas = costs[&RwasmAirDiscriminants::Cpu];
...
impl CostEstimator for ExecutionReport {
    fn estimate_area(&self) -> u64 {
        ...
        let (chips, costs) = RwasmAir::<BabyBear>::get_chips_and_costs();

        let cpu_events = self.total_instruction_count();
        total_area += (cpu_events as u64) * costs[&RwasmAirDiscriminants::Cpu];
        total_chips += 1;

        let sha_extend_events = self.syscall_counts[SyscallCode::SHA_EXTEND];
        total_area += (sha_extend_events as u64) * costs[&RwasmAirDiscriminants::Sha256Extend];
        total_chips += 1;

        let sha_compress_events = self.syscall_counts[SyscallCode::SHA_COMPRESS];
        total_area += (sha_compress_events as u64) * costs[&RwasmAirDiscriminants::Sha256Compress];
        total_chips += 1;

        let ed_add_events = self.syscall_counts[SyscallCode::ED_ADD];
        total_area += (ed_add_events as u64) * costs[&RwasmAirDiscriminants::Ed25519Add];
        total_chips += 1;

        let ed_decompress_events = self.syscall_counts[SyscallCode::ED_DECOMPRESS];
        total_area +=
            (ed_decompress_events as u64) * costs[&RwasmAirDiscriminants::Ed25519Decompress];
        total_chips += 1;

        let k256_decompress_events = self.syscall_counts[SyscallCode::SECP256K1_DECOMPRESS];
        total_area +=
            (k256_decompress_events as u64) * costs[&RwasmAirDiscriminants::K256Decompress];
        total_chips += 1;

        let secp256k1_add_events = self.syscall_counts[SyscallCode::SECP256K1_ADD];
        total_area += (secp256k1_add_events as u64) * costs[&RwasmAirDiscriminants::Secp256k1Add];
        total_chips += 1;

        let secp256k1_double_events = self.syscall_counts[SyscallCode::SECP256K1_DOUBLE];
        total_area +=
            (secp256k1_double_events as u64) * costs[&RwasmAirDiscriminants::Secp256k1Double];
        total_chips += 1;

        let keccak256_permute_events = self.syscall_counts[SyscallCode::KECCAK_PERMUTE];
        total_area += (keccak256_permute_events as u64) * costs[&RwasmAirDiscriminants::KeccakP];
        total_chips += 1;

        let bn254_add_events = self.syscall_counts[SyscallCode::BN254_ADD];
        total_area += (bn254_add_events as u64) * costs[&RwasmAirDiscriminants::Bn254Add];
        total_chips += 1;

        let bn254_double_events = self.syscall_counts[SyscallCode::BN254_DOUBLE];
        total_area += (bn254_double_events as u64) * costs[&RwasmAirDiscriminants::Bn254Double];
        total_chips += 1;

        let bls12381_add_events = self.syscall_counts[SyscallCode::BLS12381_ADD];
        total_area += (bls12381_add_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Add];
        total_chips += 1;

        let bls12381_double_events = self.syscall_counts[SyscallCode::BLS12381_DOUBLE];
        total_area +=
            (bls12381_double_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Double];
        total_chips += 1;

        let uint256_mul_events = self.syscall_counts[SyscallCode::UINT256_MUL];
        total_area += (uint256_mul_events as u64) * costs[&RwasmAirDiscriminants::Uint256Mul];
        total_chips += 1;
        ...
        total_area += (bls12381_fp_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Fp];
        total_chips += 1;
        ...
        total_area +=
            (bls12381_fp2_addsub_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Fp2AddSub];
        total_chips += 1;

        let bls12381_fp2_mul_events = self.syscall_counts[SyscallCode::BLS12381_FP2_MUL];
        total_area +=
            (bls12381_fp2_mul_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Fp2Mul];
        total_chips += 1;
        ...
        total_area += (bn254_fp_events as u64) * costs[&RwasmAirDiscriminants::Bn254Fp];
        total_chips += 1;

        let bn254_fp2_addsub_events = self.syscall_counts[SyscallCode::BN254_FP2_ADD]
            + self.syscall_counts[SyscallCode::BN254_FP2_SUB];
        total_area +=
            (bn254_fp2_addsub_events as u64) * costs[&RwasmAirDiscriminants::Bn254Fp2AddSub];
        total_chips += 1;

        let bn254_fp2_mul_events = self.syscall_counts[SyscallCode::BN254_FP2_MUL];
        total_area += (bn254_fp2_mul_events as u64) * costs[&RwasmAirDiscriminants::Bn254Fp2Mul];
        total_chips += 1;

        let bls12381_decompress_events = self.syscall_counts[SyscallCode::BLS12381_DECOMPRESS];
        total_area +=
            (bls12381_decompress_events as u64) * costs[&RwasmAirDiscriminants::Bls12381Decompress];
        total_chips += 1;

        let syscall_events = self.syscall_counts.values().sum::<u64>();
        total_area += (syscall_events as u64) * costs[&RwasmAirDiscriminants::SyscallCore];
        total_chips += 1;

        let syscall_precompile_events = self.syscall_counts.len();
        total_area +=
            (syscall_precompile_events as u64) * costs[&RwasmAirDiscriminants::SyscallPrecompile];
        total_chips += 1;
        ...
        total_area += (divrem_events as u64) * costs[&RwasmAirDiscriminants::DivRem];
        total_chips += 1;

        let addsub_events = self.opcode_counts[Opcode::ADD] + self.opcode_counts[Opcode::SUB];
        total_area += (addsub_events as u64) * costs[&RwasmAirDiscriminants::Add];
        total_chips += 1;

        let bitwise_events = self.opcode_counts[Opcode::AND]
            + self.opcode_counts[Opcode::OR]
            + self.opcode_counts[Opcode::XOR];
        total_area += (bitwise_events as u64) * costs[&RwasmAirDiscriminants::Bitwise];
        total_chips += 1;
