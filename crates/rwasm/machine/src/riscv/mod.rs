...
pub enum RwasmAir<F: PrimeField32> {
    ...
    Program(ProgramChip),
    /// An AIR that contains a preprocessed function table and a lookup for the instructions.
    Funccall(FunccallChip),
...
impl<F: PrimeField32> RwasmAir<F> {
    #[instrument("construct RwasmAir machine", level = "debug", skip_all)]
    ...
    pub fn costs() -> HashMap<RwasmAirDiscriminants, u64> {
        let (_, costs) = Self::get_chips_and_costs();
        costs
    }

    pub fn get_airs_and_costs() -> (Vec<Self>, HashMap<RwasmAirDiscriminants, u64>) {
    ...
    pub fn get_chips_and_costs() -> (Vec<Chip<F, Self>>, HashMap<RwasmAirDiscriminants, u64>) {
        ...
        >::with_lsb_rule()));
        costs.insert(RwasmAirDiscriminants::K256Decompress, k256_decompress.cost());
        chips.push(k256_decompress);

        let secp256k1_add_assign = Chip::new(RwasmAir::Secp256k1Add(WeierstrassAddAssignChip::<
            SwCurve<Secp256k1Parameters>,
        >::new()));
        costs.insert(RwasmAirDiscriminants::Secp256k1Add, secp256k1_add_assign.cost());
        ...
        costs.insert(RwasmAirDiscriminants::Bls12381Fp2AddSub, bls12381_fp2_addsub.cost());
        ...
        costs.insert(RwasmAirDiscriminants::MemoryGlobalFinal, memory_global_finalize.cost());
        chips.push(memory_global_finalize);

        let memory_local = Chip::new(RwasmAir::MemoryLocal(MemoryLocalChip::new()));
        costs.insert(RwasmAirDiscriminants::MemoryLocal, memory_local.cost());
        chips.push(memory_local);

        let memory_program = Chip::new(RwasmAir::ProgramMemory(MemoryProgramChip::default()));
        costs.insert(RwasmAirDiscriminants::ProgramMemory, memory_program.cost());
        chips.push(memory_program);

        let byte = Chip::new(RwasmAir::ByteLookup(ByteChip::default()));
        costs.insert(RwasmAirDiscriminants::ByteLookup, byte.cost());
        chips.push(byte);
    ...
    pub(crate) fn preprocessed_heights(program: &Program) -> Vec<(Self, usize)> {
        vec![
            (RwasmAir::Program(ProgramChip::default()), program.instructions.len()),
            (RwasmAir::ProgramMemory(MemoryProgramChip::default()), program.memory_image.len()),
            (RwasmAir::Funccall(FunccallChip::default()), program.index_by_offset.len()),
            (RwasmAir::ByteLookup(ByteChip::default()), 1 << 16),
        ]
    }
    ...
    pub(crate) fn core_heights(record: &ExecutionRecord) -> Vec<(Self, usize)> {
        vec![
            (RwasmAir::Cpu(CpuChip::default()), record.cpu_events.len()),
            (RwasmAir::DivRem(DivRemChip::default()), record.divrem_events.len()),
            (
                RwasmAir::Add(AddSubChip::default()),
                record.add_events.len() + record.sub_events.len(),
            ),
            (RwasmAir::Bitwise(BitwiseChip::default()), record.bitwise_events.len()),
            (RwasmAir::Mul(MulChip::default()), record.mul_events.len()),
            (RwasmAir::ShiftRight(ShiftRightChip::default()), record.shift_right_events.len()),
            (RwasmAir::ShiftLeft(ShiftLeft::default()), record.shift_left_events.len()),
            (RwasmAir::Lt(LtChip::default()), record.lt_events.len()),
            (
            ...
            (RwasmAir::SyscallCore(SyscallChip::core()), record.syscall_events.len()),
        ]
    }
    ...
    pub(crate) fn get_memory_init_final_heights(record: &ExecutionRecord) -> Vec<(Self, usize)> {
        vec![
            (
                RwasmAir::MemoryGlobalInit(MemoryGlobalChip::new(Initialize)),
                record.global_memory_initialize_events.len(),
            ),
            (
                RwasmAir::MemoryGlobalFinal(MemoryGlobalChip::new(Finalize)),
    ...
    pub(crate) fn syscall_code(&self) -> SyscallCode {
        match self {
            ...
            Self::Funccall(_) => unreachable!("Invalid for function call chip"),
        }
    }
