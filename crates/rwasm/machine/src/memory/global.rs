...
use sp1_rwasm_executor::{events::MemoryInitializeFinalizeEvent, ExecutionRecord, Program};
use sp1_stark::{
    air::{
...
impl<F: PrimeField32> MachineAir<F> for MemoryGlobalChip {
    ...
    fn generate_trace(
        ...
        let mut rows: Vec<[F; NUM_MEMORY_INIT_COLS]> = (0..memory_events.len()) // OPT: change this to par_iter
            .map(|i| {
                let MemoryInitializeFinalizeEvent { addr, mut value, shard, timestamp, used } =
                    memory_events[i];
...
//     use sp1_rwasm_executor::{programs::tests::simple_program, Executor};
//     use sp1_stark::{
