...
impl CpuChip {
    ...
    pub(crate) fn is_memory_instruction<AB: SP1AirBuilder>(
        ...
        opcode_selectors.is_i32load
            + opcode_selectors.is_i32load16s
    ...
    pub(crate) fn is_store_instruction<AB: SP1AirBuilder>(
        ...
        opcode_selectors.is_i32store8 + opcode_selectors.is_i32store16 + opcode_selectors.is_i32store
    ...
    pub(crate) fn eval_memory_store<AB: SP1AirBuilder>(
        ...
        let one = AB::Expr::one();
        let arg2_val = local.op_arg2_val();
        ...
        builder
            .when(local.selectors.is_i32store16)
        ...
        builder.when(local.selectors.is_i32store).assert_one(offset_is_zero.clone());
        ...
        let sh_expected_stored_value = Word([
            arg2_val[0] * a_is_lower_half.clone()
                + (one.clone() - a_is_lower_half.clone()) * prev_mem_val[0],
            arg2_val[1] * a_is_lower_half.clone() + (one.clone() - a_is_lower_half) * prev_mem_val[1],
            arg2_val[0] * a_is_upper_half + (one.clone() - a_is_upper_half) * prev_mem_val[2],
            arg2_val[1] * a_is_upper_half + (one.clone() - a_is_upper_half) * prev_mem_val[3],
        ]);
        builder
            .when(local.selectors.is_i32store16)
        ...
        builder
            .when(local.selectors.is_i32store)
            .assert_word_eq(mem_val.map(|x| x.into()), arg2_val.map(|x| x.into()));
    }
    ...
    pub(crate) fn eval_unsigned_mem_value<AB: SP1AirBuilder>(
        ...
        builder
            .when(local.selectors.is_i32load8s + local.selectors.is_i32load8u)
            .assert_word_eq(byte_value, local.unsigned_mem_val.map(|x| x.into()));

        // When the instruction is LH or LHU, use the lower half.
        builder
            .when(local.selectors.is_i32load16s+ local.selectors.is_i32load16u)
            .assert_zero(memory_columns.offset_is_one + memory_columns.offset_is_three);

        // When the instruction is LW, ensure that the offset is zero.
        builder.when(local.selectors.is_i32load).assert_one(offset_is_zero.clone());
        ...
        builder.when(local.selectors.is_i32load).assert_word_eq(mem_val, local.unsigned_mem_val);
    }
    ...
    pub(crate) fn eval_most_sig_byte_bit_decomp<AB: SP1AirBuilder>(
        ...
        builder.when(local.selectors.is_i32load8s).assert_eq(recomposed_byte.clone(), unsigned_mem_val[0]);
        builder.when(local.selectors.is_i32load16s).assert_eq(recomposed_byte, unsigned_mem_val[1]);
    }
