...
__SP1_HOSTDEV__ __SP1_INLINE__ array_t<uint8_t, 8> u64_to_le_bytes(uint64_t n) {
  return {
      (uint8_t)(n >> 8 * 0), (uint8_t)(n >> 8 * 1), (uint8_t)(n >> 8 * 2),
      (uint8_t)(n >> 8 * 3), (uint8_t)(n >> 8 * 4), (uint8_t)(n >> 8 * 5),
      (uint8_t)(n >> 8 * 6), (uint8_t)(n >> 8 * 7),
  };
}

/// Shifts a byte to the right and returns both the shifted byte and the bits that carried.
__SP1_HOSTDEV__ __SP1_INLINE__ std::tuple<uint8_t, uint8_t> shr_carry(
    uint8_t input, uint8_t rotation) {
...
template <class F>
__SP1_HOSTDEV__ __SP1_INLINE__ void write_word_from_u32(
    Word<decltype(F::val)>& word, const uint32_t value) {
...
template <class F>
__SP1_HOSTDEV__ __SP1_INLINE__ void write_word_from_u32_v2(
    Word<F>& word, const uint32_t value) {
...
__SP1_HOSTDEV__ __SP1_INLINE__ void word_from_le_bytes(
    Word<decltype(F::val)>& word, const array_t<uint8_t, 4> bytes) {
  // Coercion to `uint8_t` truncates the number.
