...
fn main() {
    ...
    let maximal_shapes: BTreeMap<usize, Vec<Shape<RwasmAirId>>> = serde_json::from_slice(
    ...
    for (log2_shard_size, shapes) in maximal_shapes.iter() {
        ...
        for shape in shapes.iter() {
            for log2_memory_height in args.log2_memory_heights.iter() {
                ...
                let log2_gap_from_21 = 21 - small_shape.log2_height(&RwasmAirId::Cpu).unwrap();
                let min_log2_height_threshold = 18 - log2_gap_from_21;
                for air in RwasmAirId::core() {
                ...
                small_shape.insert(RwasmAirId::MemoryGlobalInit, *log2_memory_height);
                small_shape.insert(RwasmAirId::MemoryGlobalFinalize, *log2_memory_height);
                small_shape.insert(RwasmAirId::Global, log2_memory_height + 1);
