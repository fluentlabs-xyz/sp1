...
use rwasm_machine::{io::SP1Stdin, riscv::RwasmAir, shape::CoreShapeConfig, utils::setup_logger};
...
fn test_shape_fixing(
    ...
    while !finished {
        ...
        for mut record in records {
            let _ = record.defer();
            let heights = RwasmAir::<BabyBear>::core_heights(&record);
            println!("heights: {:?}", heights);

            shape_config.fix_shape(&mut record).unwrap();

            if record.contains_cpu()
                && record.shape.unwrap().height(&RwasmAirId::Cpu).unwrap() > opts.shard_size
            {
