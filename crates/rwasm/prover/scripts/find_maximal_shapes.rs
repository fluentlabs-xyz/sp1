...
use rwasm_executor::{Executor, Program, RwasmAirId, SP1Context};
use rwasm_machine::{io::SP1Stdin, rwasm::RwasmAir, utils::setup_logger};
    ...
    while !finished {
        ...
        for mut record in records {
            if record.contains_cpu() {
                let _ = record.defer();
                let core_shape: Shape<RwasmAirId> = RwasmAir::<BabyBear>::core_heights(&record)
                    .into_iter()
...
fn insert(inner: &mut Vec<Shape<RwasmAirId>>, element: Shape<RwasmAirId>) {
...
fn main() {
    ...
    let mut all_maximal_shapes: BTreeMap<usize, Vec<Shape<RwasmAirId>>> =
