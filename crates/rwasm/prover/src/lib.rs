...
use sp1_rwasm_executor::{ExecutionError, ExecutionReport, Executor, Program, SP1Context};
...
impl<C: SP1ProverComponents> SP1Prover<C> {
    ...
    /// Creates a proving key and a verifying key for a given RISC-V ELF.
    #[instrument(name = "setup_program", level = "debug", skip_all)]
    pub fn setup_program(&self, program: &mut Program) -> (SP1ProvingKey, SP1VerifyingKey) {
        if let Some(core_shape_config) = &self.core_shape_config {
            core_shape_config.fix_preprocessed_shape(program);
        }
        let (pk, vk) = self.core_prover.setup(program);
        let vk = SP1VerifyingKey { vk };
        let pk = SP1ProvingKey { pk: pk.to_host(), elf: vec![], vk: vk.clone() };
        (pk, vk)
    }
    ...
    pub fn prove_core<'a>(
        ...
        let (proof, public_values_stream, cycles) = sp1_rwasm_machine::utils::prove_with_context::<
            _,
            C::CoreProver,
        >(
            &self.core_prover,
    ...
    /// Generate shard proofs which split up and prove the valid execution of a RISC-V program with
    /// the core prover. Uses the provided context.
    #[instrument(name = "prove_core_program", level = "info", skip_all)]
    pub fn prove_core_program<'a>(
        &'a self,
        pk: &SP1ProvingKey,
        program: Program,
        stdin: &SP1Stdin,
