...
use std::{fs::File, io::BufWriter, ops::Shl};
...
use crate::{
    ...
    CoreAirId, MaximalShapes, Opcode, Program, Register, RwasmAirId, FUNFRAMEP_START, SP_START,
};
use bincode::de;
use num::Signed;
use rwasm::{
...
mod tests {
    ...
    fn build_elf_branching() {
        ...
        let mut runtime = Executor::new(program, SP1CoreOpts::default());
        runtime.run().unwrap();
        assert_eq!(runtime.state.memory.get(runtime.state.sp).unwrap().value, 3);
    }
